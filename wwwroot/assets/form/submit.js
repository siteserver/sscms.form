  ...
  var $vue = new Vue({
    ...
    methods: {
      ...
      imageUploaded: function(error, file) {
        if (!error) {
          ...
          var style = _.find(this.styleList, function(o) { return o.id === res.fieldId; });
    ...
    created: function () {
      ...
      $api.post(this.apiUrl + '/ss.form/' + this.siteId + '/' + this.formId + '/actions/get')
        .then(function (res) {
          $this.styleList = res.data.value;
          for (var i = 0; i < $this.styleList.length; i++) {
            var style = $this.styleList[i];
            if (style.fieldType === 'CheckBox' || style.fieldType === 'SelectMultiple') {
              style.value = [];
            }
          }
